...
yield_data <- hyperfarm_2023_yield %>%
  ...
  # Convert columns to appropriate types if necessary
   mutate(
    ...
    CUT = as.factor(CUT),
    DRY_WEIGHT_1_HA   = as.numeric(DRY_WEIGHT_1_HA)
    ) %>%
...
yield_data %>% 
  ...
  relocate(DRY_WEIGHT_1_HA, PV_SETUP, SUB_STRIP_LOCATION, SUB_STRIP_LOCATION_BETWEEN_PV_vs_REFERENCE) 
...
yield_data %>% 
  ...
  relocate(DRY_WEIGHT_1_HA, PV_SETUP, SUB_STRIP_LOCATION, SUB_STRIP_LOCATION_BETWEEN_PV_vs_REFERENCE) %>% 
  summarize(
    mean_dry_weight = mean(DRY_WEIGHT_1_HA, na.rm = TRUE),
    sd_dry_weight = sd(DRY_WEIGHT_1_HA, na.rm = TRUE),
    median_dry_weight = median(DRY_WEIGHT_1_HA, na.rm = TRUE),
    count = n(),
    .groups = 'drop'
  )
...
global_means <- yield_data %>%
  ...
  summarize(global_mean_dry_weight = mean(DRY_WEIGHT_1_HA, na.rm = TRUE), .groups = 'drop')  # Calculate global mean
...
yield_data_with_global_mean %>%
  ...
  select(PRODUCT, PV_SETUP, LOCATION, DRY_WEIGHT_1_HA, global_mean_dry_weight) |>   # Select relevant columns for display
  filter(LOCATION == "OUTSIDE_PV")
...
lmm_hyperfarm <- lmerTest::lmer(DRY_WEIGHT_1_HA ~ PV_SETUP + LOCATION + PRODUCT + 
                                  PV_SETUP:LOCATION +
                                  PV_SETUP:PRODUCT +
                                  (1 | SUB_STRIP), 
                                data = yield_data)
...
yield_data_lm <- yield_data |> 
  ...
  relocate(PV_SETUP, SETUP_BETWEEN_PV_vs_REFERENCE, SUB_STRIP_LOCATION_BETWEEN_PV_vs_REFERENCE, PRODUCT, LOCATION, DRY_WEIGHT_1_HA) |> glimpse()
```
...
lm_hyperfarm_substrips_simple <- lm(DRY_WEIGHT_1_HA ~ SETUP_BETWEEN_PV_vs_REFERENCE + SUB_STRIP_LOCATION_BETWEEN_PV_vs_REFERENCE + PRODUCT, 
                                    data = yield_data_lm)

# Full model with interactions
lm_hyperfarm_substrips_full <- lm(DRY_WEIGHT_1_HA ~ SETUP_BETWEEN_PV_vs_REFERENCE + SUB_STRIP_LOCATION_BETWEEN_PV_vs_REFERENCE + PRODUCT + 
...
overall_means <- yield_data_lm %>%
  ...
  relocate(SETUP_BETWEEN_PV_vs_REFERENCE, SUB_STRIP_LOCATION_BETWEEN_PV_vs_REFERENCE, PRODUCT, LOCATION, DRY_WEIGHT_1_HA) |> 

    summarise(mean_yield = mean(DRY_WEIGHT_1_HA, na.rm = FALSE))
...
yield_data  %>%
  mutate(
    PV_SETUP_2 = case_when(
      ...
      PV_SETUP == "FIXED_TILTED" & LOCATION == "BETWEEN_PV" ~ "SOUTH-ORIENTED_BETWEEN_PV",
      PV_SETUP == "FIXED_TILTED" & LOCATION == "OUTSIDE_PV" ~ "SOUTH-ORIENTED_OUTSIDE_PV",
      TRUE ~ NA_character_  # Handle any other cases if they exist
    )
  ) |> relocate(PV_SETUP_2, DRY_WEIGHT_1_HA) |> 
  ...
  # Join summarized yield data with pv_data
  inner_join(pv_data, by = c("PV_SETUP_2" = "PV_SETUP")) 
...
pv_data_preproc <- pv_data %>%
  ...
  mutate(
    PV_SETUP_2 = case_when(
      ...
      PV_SETUP_2 == "SOUTH-ORIENTED" ~ "SOUTH-ORIENTED_BETWEEN_PV",
...
calculate_a_LER <- function(combined_ler_data, use_global_mean = FALSE) {
  ...
  pv_setup_ler <- combined_ler_data %>%
    ...
    # Calculate the crop yield ratio correctly
    mutate(
      crop_yield_ratio = case_when(
        
        # For the SOUTH-ORIENTED_BETWEEN_PV setup
        PV_SETUP_2 == "SOUTH-ORIENTED_BETWEEN_PV" ~ mean_dry_weight / 
          ifelse(use_global_mean, 
                 global_mean_dry_weight,  # Use global mean dry weight as the reference
                 combined_ler_data$mean_dry_weight[combined_ler_data$PV_SETUP_2 == "SOUTH-ORIENTED_OUTSIDE_PV"]  # Use the specific mean dry weight from "SOUTH-ORIENTED_OUTSIDE_PV"
          ),
        
        # For the VERTICAL_BETWEEN_PV setup
        PV_SETUP_2 == "VERTICAL_BETWEEN_PV" ~ mean_dry_weight / 
          ifelse(use_global_mean, 
                 global_mean_dry_weight,  # Use global mean dry weight as the reference
                 combined_ler_data$mean_dry_weight[combined_ler_data$PV_SETUP_2 == "VERTICAL_OUTSIDE_PV"]  # Use the specific mean dry weight from "VERTICAL_OUTSIDE_PV"
          ),
        
        # For any other PV setups, return NA as crop_yield_ratio is not applicable
        TRUE ~ NA_real_
      ),
      
      # Calculate the land adjustment factor using the formula with correct units
      land_adjustment_factor = case_when(
        PV_SETUP_2 == "SOUTH-ORIENTED_BETWEEN_PV" ~ ((TOTAL_AREA_OCCUPIED_M2 / 10000 - LL_HA) / (TOTAL_AREA_OCCUPIED_M2 / 10000)),
...
# Define a function to add the required columns to any dataset
add_relative_yield_columns <- function(data) {
  data %>%
    mutate(
      # Calculate relative crop yield for vertical and south-oriented setups
      relative_crop_yield_vertical = ifelse(PV_SETUP_2 == "VERTICAL_BETWEEN_PV", 
                                            (mean_dry_weight / data$mean_dry_weight[data$PV_SETUP_2 == "VERTICAL_OUTSIDE_PV"]) * 100, 
                                            NA_real_),
      relative_crop_yield_south_oriented = ifelse(PV_SETUP_2 == "SOUTH-ORIENTED_BETWEEN_PV", 
                                                  (mean_dry_weight / data$mean_dry_weight[data$PV_SETUP_2 == "SOUTH-ORIENTED_OUTSIDE_PV"]) * 100, 
                                                  NA_real_),
      
      # Calculate relative energy yield for vertical and south-oriented setups
      relative_energy_yield_vertical = ifelse(PV_SETUP_2 == "VERTICAL_BETWEEN_PV", 
                                              (ELECTRICITY_OUTPUT_MWH_HA_YR / data$ELECTRICITY_OUTPUT_MWH_HA_YR[data$PV_SETUP_2 == "REFERENCE"]) * 100, 
                                              NA_real_),
      relative_energy_yield_south_oriented = ifelse(PV_SETUP_2 == "SOUTH-ORIENTED_BETWEEN_PV", 
                                                    (ELECTRICITY_OUTPUT_MWH_HA_YR / data$ELECTRICITY_OUTPUT_MWH_HA_YR[data$PV_SETUP_2 == "REFERENCE"]) * 100, 
                                                    NA_real_),
      
      # Calculate land loss percentages for vertical and south-oriented setups
      land_loss_vertical = ifelse(PV_SETUP_2 == "VERTICAL_BETWEEN_PV", 
                                  (LL_HA / (TOTAL_AREA_OCCUPIED_M2 / 10000)) * 100, 
                                  NA_real_),
      land_loss_south_oriented = ifelse(PV_SETUP_2 == "SOUTH-ORIENTED_BETWEEN_PV", 
                                        (LL_HA / (TOTAL_AREA_OCCUPIED_M2 / 10000)) * 100, 
                                        NA_real_)
    )
}

# Function to calculate and return the full dataset with a-LER, relative yield, and land loss components
calculate_full_LER <- function(data, use_global_mean = FALSE) {
  calculated_data <- calculate_a_LER(data, use_global_mean = use_global_mean)
  final_data <- add_relative_yield_columns(calculated_data)
...
rename_pv_setup <- function(data) {
  data %>%
    mutate(
      PV_SETUP_2 = case_when(
        PV_SETUP_2 == "SOUTH-ORIENTED_BETWEEN_PV" ~ "SOUTH-ORIENTED",
...
folder_path <- "C:/Users/au759124/OneDrive - Aarhus universitet/Documents/V4F_agrivoltaics/FIGURES/LER"
...
a_LER_clover_plot_global_mean <- plot_combined_a_LER(a_LER_clover_global_mean_clean, "Grass-Clover")
...
combined_a_LER_global_mean <- bind_rows(
  ...
  a_LER_clover_global_mean %>% mutate(Product = "Grass-Clover")
)
...
combined_a_LER_pv_sys <- bind_rows(
  ...
  a_LER_grain_pv_sys_mean %>% mutate(Product = "Grass-Clover")
)
```
...
add_relative_yield_columns <- function(data, use_global_mean = FALSE) {
  ...
  # Calculate global mean if necessary
  if (use_global_mean) {
    global_means <- data %>%
      filter(grepl("_OUTSIDE_PV$", PV_SETUP_2)) %>%
      group_by(Product) %>%
      summarise(global_mean_dry_weight = mean(mean_dry_weight, na.rm = TRUE))
  ...
  data %>%
    mutate(
      # Calculate relative crop yield for vertical setups
      relative_crop_yield_vertical = ifelse(
        PV_SETUP_2 == "VERTICAL_BETWEEN_PV",
        (mean_dry_weight / ifelse(use_global_mean, global_mean_dry_weight, data$mean_dry_weight[data$PV_SETUP_2 == "VERTICAL_OUTSIDE_PV"])) * 100,
        NA_real_
      ),
      # Calculate relative crop yield for south-oriented setups
      relative_crop_yield_south_oriented = ifelse(
        PV_SETUP_2 == "SOUTH-ORIENTED_BETWEEN_PV",
        (mean_dry_weight / ifelse(use_global_mean, global_mean_dry_weight, data$mean_dry_weight[data$PV_SETUP_2 == "SOUTH-ORIENTED_OUTSIDE_PV"])) * 100,
        NA_real_
      ),
      # Calculate relative energy yield for vertical setups
      relative_energy_yield_vertical = ifelse(
        PV_SETUP_2 == "VERTICAL_BETWEEN_PV",
        (ELECTRICITY_OUTPUT_MWH_HA_YR / data$ELECTRICITY_OUTPUT_MWH_HA_YR[data$PV_SETUP_2 == "REFERENCE"]) * 100,
        NA_real_
      ),
      # Calculate relative energy yield for south-oriented setups
...
# GLOBAL MEAN
####################################
combined_a_LER_relative_yield_global_mean <- add_relative_yield_columns(data = combined_a_LER_global_mean,
                                                                        use_global_mean = TRUE) |> 
  rename_pv_setup() 

# View the updated combined_a_LER dataset
...
# PV SETUP-SPECIFIC MEAN
####################################
combined_a_LER_relative_yield_pv_sys <- add_relative_yield_columns(combined_a_LER_pv_sys,
                                                                   use_global_mean = TRUE) |> 
  rename_pv_setup() 


# View the updated combined_a_LER dataset
combined_a_LER_relative_yields_pv_sys_table <- combined_a_LER_relative_yield_pv_sys %>%
  select(PV_SETUP_2, Product, a_LER, mean_dry_weight, ELECTRICITY_OUTPUT_MWH_HA_YR, 
         relative_crop_yield_vertical, relative_crop_yield_south_oriented, 
         relative_energy_yield_vertical, relative_energy_yield_south_oriented, 
         land_loss_vertical, land_loss_south_oriented)
  ...
  data %>%
    ...
    mutate(
      ...
      relative_crop_yield = case_when(
        ...
        grepl("SOUTH-ORIENTED", PV_SETUP_2) ~ relative_crop_yield_south_oriented,
        TRUE ~ NA_real_
      ),
      ...
      relative_energy_yield = case_when(
        ...
        grepl("SOUTH-ORIENTED", PV_SETUP_2) ~ relative_energy_yield_south_oriented,
        TRUE ~ NA_real_
      ),
      # Combine land loss into one column
      land_loss = case_when(
        grepl("VERTICAL", PV_SETUP_2) ~ land_loss_vertical,
        grepl("SOUTH-ORIENTED", PV_SETUP_2) ~ land_loss_south_oriented,
        TRUE ~ NA_real_
      ),
...
clean_grass_clover_table_global_mean <- create_clean_product_table(combined_a_LER_relative_yields_global_mean_table, "Grass-Clover")
```
...
grain_data_global_mean <- combined_a_LER_relative_yields_for_table_global_mean %>%
  filter(Product == "Grain", 
    ...
    mutate(
      PV_SETUP_2 = case_when(
        PV_SETUP_2 == "SOUTH-ORIENTED" ~ "Tilted south-oriented",
        PV_SETUP_2 == "VERTICAL" ~ "Vertical",
        PV_SETUP_2 == "REFERENCE" ~ "REFERENCE",  # Keep REFERENCE unchanged
        TRUE ~ PV_SETUP_2  # Leave other values unchanged if any
      )
    ) |> 
...
output_file_path <- "C:/Users/au759124/OneDrive - Aarhus universitet/Documents/V4F_agrivoltaics/FIGURES/LER/combined_LER_table_modified.xlsx"
